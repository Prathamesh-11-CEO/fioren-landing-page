...
function leadEmailHtml({ name, email, phone, address, qty, amount }) {
  const rupees = amount ? (amount / 100).toLocaleString('en-IN', { style: 'currency', currency: 'INR' }) : '—';
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,.08);">
        ...
        <tr>
          <td style="background:#1C1614;padding:28px 40px;text-align:center;">
            <div style="font-family:Georgia,serif;font-size:22px;font-weight:800;letter-spacing:4px;color:#ffffff;">FIOREN<sup style="font-size:11px;">®</sup></div>
            <div style="font-size:12px;color:#E8A33D;letter-spacing:1px;margin-top:4px;font-weight:700;">CHECKOUT LEAD — NOT YET PURCHASED</div>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:36px 40px;">
            ...
            <h2 style="margin:0 0 4px;font-family:Georgia,serif;font-size:22px;color:#1C1614;">Checkout Started 👀</h2>
            <p style="margin:0 0 28px;font-size:14px;color:#6B5C55;">This visitor entered their details on checkout but has not completed payment yet. Reach out to help them finish the order.</p>

            <!-- Customer -->
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
              <tr><td style="background:#FCEFD8;border-radius:8px;padding:20px 24px;">
                <div style="font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#C4A897;margin-bottom:14px;">Customer Details</div>
                <table width="100%" cellpadding="4" cellspacing="0" style="font-size:13px;color:#1C1614;">
                  <tr><td style="color:#6B5C55;width:130px;">Name</td><td><strong>${escHtml(name)}</strong></td></tr>
                  <tr><td style="color:#6B5C55;">Email</td><td>${escHtml(email)}</td></tr>
                  <tr><td style="color:#6B5C55;">Phone</td><td><strong>${escHtml(phone)}</strong></td></tr>
                </table>
              </td></tr>
            </table>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
              <tr><td style="background:#EDEAE6;border-radius:8px;padding:20px 24px;">
                <div style="font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#C4A897;margin-bottom:10px;">Address (partial)</div>
                <div style="font-size:13px;color:#1C1614;line-height:1.7;">${escHtml(address)}</div>
              </td></tr>
            </table>` : ''}
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
              <tr><td style="background:#EDEAE6;border-radius:8px;padding:20px 24px;">
                <div style="font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#C4A897;margin-bottom:14px;">Cart</div>
                <table width="100%" cellpadding="4" cellspacing="0" style="font-size:13px;color:#1C1614;">
                  <tr><td style="color:#6B5C55;width:130px;">Product</td><td>FIOREN® Advanced Anti-Ageing Renewal Cream (50g)</td></tr>
                  <tr><td style="color:#6B5C55;">Quantity</td><td>${escHtml(String(qty))}</td></tr>
                  <tr><td style="color:#6B5C55;">Cart Value</td><td><strong style="color:#7D5C4E;">${rupees}</strong></td></tr>
                </table>
              </td></tr>
            </table>
...
async function sendLeadEmail({ name, email, phone, address, qty, amount }) {
  const transporter = createTransporter();
  await transporter.sendMail({
    from:    `"FIOREN® Leads" <${process.env.GMAIL_USER}>`,
    to:      process.env.GMAIL_USER,
    subject: `CHECKOUT LEAD — NOT YET PURCHASED — ${name || 'Customer'}`,
    html:    leadEmailHtml({ name, email, phone, address, qty, amount }),
  });
}
